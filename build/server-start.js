...
var srcPath = path.join(projectRootPath, 'server')
var appPath = path.join(projectRootPath, 'serverdist')
var fs = require('fs')
...
var watcher = chokidar.watch(path.join(__dirname, '../server'))

watcher.on('ready', function () {
  log('Compiling...'.green)
  babelCliDir({ outDir: 'serverdist/', retainLines: true, sourceMaps: true }, [ 'server/' ]) // compile all when start
  require('../serverdist/app.js') // start app
  log('♪ App Started'.green)

  watcher
    .on('add', function (absPath) {
      compileFile('server/', 'serverdist/', path.relative(srcPath, absPath), cacheClean)
    })
